...
export default function Output({ body }) {
  ...
  const runSQL = async () => {
    ...
    if (data.success && data.data) {
      ...
      setError(`SQL query succesfully executed. However, result set is empty.`);
    } else {
      setError(data.error);
    }
  };
      ...
      <div className="flex justify-between items-center">
        ...
        <Button onClick={runSQL}>
          {isLoading && (
            <svg
              role="status"
              className="w-6 h-6 text-white animate-spin dark:text-darkBGColor fill-darkBGColor dark:fill-darkTextColor"
              viewBox="0 0 100 101"
              fill="none"
